        ...
        <table class="text-left table-fixed min-w-ma">
            ...
            @foreach($collection as $attributes)
                <tr class="bg-neutral-primary border-b-3 border-gray-700">
                    <th scope="row" class="px-6 py-4 font-medium text-heading whitespace-nowrap {{ $textColor[$attributes->type_id] }}">
                    ...
                    <td class="px-6 py-4">
                        {{ \Carbon\Carbon::parse($attributes->updated_at)->format('d/m/Y') }}
                    </td>
                    <td class="flex flex-row px-6 py-4">
                        <a href="{{ route($edit, $attributes->id) }}" class="w-fit justify-center">
                            <button type="button" class="bg-transparent flex items-center rounded-lg px-4 py-2 text-sm hover:bg-yellow-500" role="menuitem">
                        ...
                        <button id="destroy-button"
                                onclick="return confirm('Você deseja excluir esta categoria permanentemente?');"
                                type="submit"
                                form="destroy"
...
<script>
    ...
    //     confirm("Você deseja excluir esta categoria permanentemente?");
    // })
</script>
