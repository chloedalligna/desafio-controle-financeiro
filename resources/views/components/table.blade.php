<div class="flex justify-center flex-col w-7xl h-full overflow-scroll text-white bg-white dark:bg-gray-800  rounded-xl bg-clip-border">
    <table class="w-full text-left table-auto min-w-max">
        <thead class="bg-gray-700 ">
            <tr>
                ...
                @endforeach
                <th scope="col" class="p-4 border-b border-blue-gray-50">
        ...
        <tbody>
            @foreach($collection as $attributes)
                <tr class="bg-neutral-primary border-b border-default">
                    @foreach($attributes->except(['id', '_token', 'deleted_at']) as $key => $value)
                        @switch($key)
                            @case('name')
                            ...
                            @case('category_id')
                                <td class="px-3 py-4">
                                    <a class="rounded-2xl p-2 {{ $backgroundColor[$attributes->category->type_id] }}">
                            ...
                            @case('value')
                                <td class="px-6 py-4">
                                   R$ {{ Number::format($value, 2, null, 'pt_BR') }}
                                </td>
                                @break
                            @case('description')
                                <td class="px-6 py-4">
                    ...
                    @endforeach
                    <td>
                        <a href="{{ route($edit, $attributes->id) }}" class="w-full justify-center">
                            <button type="button" class="bg-transparent flex items-center ml-3 rounded-lg px-4 py-2 text-sm hover:bg-yellow-500" role="menuitem">
